...
static void addName(nameCtx h,
    ...
    char *dst;
    char *newStr;
    newStr = (char *)MEM_NEW(h->g, length * sizeof(char));
    NameRecord *rec;
    int index;
...
static void addWinDfltName(nameCtx h,
                           unsigned short nameId, int length, const char *str) {
    addName(h,
            HOT_NAME_MS_PLATFORM,
            HOT_NAME_MS_UGL,
            HOT_NAME_MS_ENGLISH,
...
void nameNew(hotCtx g) {
    nameCtx h = (nameCtx)MEM_NEW(g, sizeof(struct nameCtx_));
...
static void addStdNames(nameCtx h, int win, int mac) {
...

    {
        ...
        if (g->font.version.client == NULL) {
            snprintf(buf, sizeof(buf), "Version %.3f;%s %ld.%ld.%ld",
                    dFontVersion,
                    idTag,
                    (g->version >> 16) & 0xff, (g->version >> 8) & 0xff, g->version & 0xff);
        } else {
            snprintf(buf, sizeof(buf), "Version %.3f;%s %ld.%ld.%ld;%s",
                    dFontVersion,
                    idTag,
                    (g->version >> 16) & 0xff, (g->version >> 8) & 0xff, g->version & 0xff,
    ...
    addStdNamePair(h, win, mac, HOT_NAME_FONTNAME,
                   strlen(g->font.FontName.c_str()), g->font.FontName.c_str());
}
...
static void fillNames(nameCtx h) {
    ...
    const char *style = styles[h->g->font.flags & 0x3];
    ...
    int doOTSpecName4 = 1;  // !(h->g->convertFlags & HOT_USE_OLD_NAMEID4); write Windows name ID 4 correctly in Win platform.
    int omitMacNames = (h->g->convertFlags & HOT_OMIT_MAC_NAMES);    /* omit all Mac platform names. */
    ...
    if (NULL != getName(h, MATCH_ANY,
                        ...
                        MATCH_ANY,
                        HOT_NAME_FAMILY)) {
        doWarning = 1; /* If the user entered any explicit HOT_NAME_FAMILY names,             */
                       /* we should warn of any HOT_NAME_PREF_FAMILY that are not matched by  */
    ...
    /* Any MS UGL HOT_NAME_PREF_SUBFAMILY names must be explicitly defined; */
    /* else we will add  only the HOT_NAME_SUBFAMILY values.                */
    ...
    if (!doOTSpecName4) {
        ...
        addWinDfltName(h, HOT_NAME_FULL, strlen(h->g->font.FontName.c_str()),
                       h->g->font.FontName.c_str());
    } else {
        ...
        index = enumNames(h, index,
                          ...
                          MATCH_ANY,
                          HOT_NAME_FULL);
        if (index == -1) { /* no predefined MS UGL HOT_NAME_FAMILY recs */
            index = 0;
            for (;;) {
                ...
                if (Subfamily == NULL) {
                    ...
                    snprintf(Full, sizeof(Full), "%s %s", Family, Subfamily);
                    addName(h,
                            HOT_NAME_MS_PLATFORM,
                            rec->platspecId,
                            rec->languageId,
    ...
    if (!omitMacNames) {
        ...
        /* For V1 syntax, any HOT_NAME_PREF_SUBFAMILY must be copied to the HOT_NAME_SUBFAMILY      */
        /* entries, so that the HOT_NAME_PREF_SUBFAMILY entries will later be eliminated. We didn't */
        ...
        /* Now for all Mac HOT_NAME_FAMILY names, fill in the HOT_NAME_SUBFAMILY names.      */
        /* These may not yet be filled in even for V1 syntax, despite the loop above. If the */
        /* user specified Mac HOT_NAME_PREF_FAMILY names but not HOT_NAME_PREF_SUBFAMILY     */
        /* names, there will still be no HOT_NAME_SUBFAMILY entries.                         */
...
static int CTL_CDECL cmpNames(const void *first, const void *second) {
    const NameRecord *a = (const NameRecord *)first;
    const NameRecord *b = (const NameRecord *)second;
